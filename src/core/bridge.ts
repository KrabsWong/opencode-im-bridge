    ...
    try {
      switch (action) {
        ...
        case "select_session":
          ...
          break
        case "tui_command":
          await this.handleTuiCommandCallback(callback.user.id, requestId, callback.messageId)
          break
        default:
...
`
   ...
   */
  private async handleTuiCommandCallback(userId: string, command: string, messageId?: string): Promise<void> {
    // Handle autotitle separately
    if (command === "autotitle") {
      await this.executeAutotitle(userId, messageId)
      return
    }
    ...
    if (validCommands.includes(command)) {
      await this.executeTuiCommand(command, userId, messageId)
    } else {
      // If messageId is provided, edit the original message; otherwise send new
      const errorText = `<b>❌ 未知操作</b>: ${command}`
      if (messageId) {
   ...
   */
  private async executeTuiCommand(command: string, userId?: string, messageId?: string): Promise<void> {
    // Helper function to send or edit message
    const sendResult = async (text: string, isError: boolean = false) => {
   ...
   */
  private async executeAutotitle(userId: string, messageId?: string): Promise<void> {
    // Helper function to send or edit message
    ...
    try {
      ...
      // Show "generating" status by editing the original message
      await sendResult(
        `<b>📝 正在生成标题...</b>\n分析会话内容，请稍候`
      )
...
3. 只返回标题文字本身，不要有引号、解释或格式`
      ...
      // Notify user by editing the original message
      await sendResult(
...
当前没有可用的会话。`,
  ...
  private async handleDirectMessage(text: string, userId: string, _message: IMMessage): Promise<void> {
    ...
    try {
      ...
      // Warn if session is not active (send separate warning message)
      if (sessionStatus === "idle") {
        const warningText = `${sessionPrefix}\n` +
          `**⚠️ 警告: 会话可能已关闭或不活跃**\n` +
          `状态: ${this.getStatusLabel(sessionStatus)}\n\n` +
          `消息仍会尝试发送，但可能无法得到响应。\n` +
          `建议使用 /sessions 选择活跃的会话。`
        const warningResult = markdownToEntities(warningText)
        await this.sendMessage({
          text: warningResult.text,
          parseMode: "entities",
          entities: warningResult.entities,
        })
        // Continue anyway but warn user
      }

      // Send initial "processing" message with session prefix, record messageId for later update
      const processingText = `${sessionPrefix}\n**⏳ 正在处理请求...**`
      const processingResult1 = markdownToEntities(processingText)
      const processingResult = await this.sendMessage({
        text: processingResult1.text,
        parseMode: "entities",
        entities: processingResult1.entities,
      })
      processingMessageId = processingResult.messageId
      ...
      if (response) {
        ...
        if (errorInfo) {
          ...
          if (errorName?.includes("Aborted") || errorName?.includes("Interrupt")) {
            // Update the processing message to show interrupted status
            if (processingMessageId && this.adapter.editMessage) {
              const interruptText = `${sessionPrefix}\n**⏹️ 会话已中断**\n\n任务被用户手动中断或取消。`
              const interruptResult = markdownToEntities(interruptText)
              await this.adapter.editMessage(processingMessageId, {
                text: interruptResult.text,
                parseMode: "entities",
                entities: interruptResult.entities,
              })
            } else {
              const interruptText2 = `${sessionPrefix}\n**⏹️ 会话已中断**\n\n任务被用户手动中断或取消。`
              const interruptResult2 = markdownToEntities(interruptText2)
              await this.sendMessage({
                text: interruptResult2.text,
                parseMode: "entities",
                entities: interruptResult2.entities,
              })
            }
            return
          }
      ...
      // Use new entities mode for better formatting (no escaping needed)
      this.logger.debug("Sending AI response using entities mode", { 
        text: displayResponse.substring(0, 500),
        length: displayResponse.length 
      })

      // Check for duplicate response before sending
      const responseHash = `${sessionPrefix}:${displayResponse}`
      if (this.isDuplicateResponse(sessionId, responseHash)) {
        this.logger.info(`Duplicate response detected for session ${sessionId}, skipping send`)
        if (processingMessageId && this.adapter.editMessage) {
          const duplicateText = `${sessionPrefix}\n**🦀 蟹老板说**\n\n[重复响应，已跳过]`
          const duplicateResult = markdownToEntities(duplicateText)
          await this.adapter.editMessage(processingMessageId, {
            text: duplicateResult.text,
            parseMode: "entities",
            entities: duplicateResult.entities,
          })
        }
        return
      }

      // Send using entities mode
      await this.sendMarkdownWithEntities(
        sessionPrefix,
        '🦀 蟹老板说',
        displayResponse,
        {
          editMessageId: processingMessageId || undefined
        }
      )
    } catch (error) {
      ...
      // Try to update the processing message with error, or send new error message
      const sendError = async (errorText: string) => {
        if (processingMessageId && this.adapter.editMessage) {
          await this.adapter.editMessage(processingMessageId, {
            text: errorText,
            parseMode: "html",
      ...
      try {
        if (responseText) {
          const fallbackText = `${errorPrefix}\n[🦀 蟹老板说 - 格式渲染失败，显示原始内容]\n\n${responseText}`
          if (processingMessageId && this.adapter.editMessage) {
