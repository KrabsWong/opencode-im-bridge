...
import { markdownToEntities, splitEntities, type TelegramEntity as MarkdownEntity } from "../core/markdown-entities.js"
...
export class TelegramAdapter implements IMAdapter {
  ...
  async sendMessage(message: IMOutgoingMessage): Promise<{ messageId: string }> {
    ...
    if (message.parseMode === "entities") {
      // 使用 entities 模式 - 无需转义，直接发送纯文本 + entities
      const entities = message.entities || []
      body.text = message.text.substring(0, 4096)
      body.entities = this.convertToTelegramEntities(entities)
      // 不设置 parse_mode，使用 entities
    } else {
      // 传统模式 - HTML 或 MarkdownV2
      const text = message.text
      body.text = text.substring(0, 4096)
    ...
    adapterLogger.info(`[TelegramAdapter] Final text being sent: ${JSON.stringify(body.text).substring(0, 200)}`)
  ...
  /**
   * 将内部 Entity 格式转换为 Telegram API 格式
   */
  private convertToTelegramEntities(entities: MarkdownEntity[]): Array<{
    type: string
    offset: number
    length: number
    url?: string
